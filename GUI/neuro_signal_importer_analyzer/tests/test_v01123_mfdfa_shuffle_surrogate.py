...
def _make_recording(root: Path, n_samples: int = 7000, n_channels: int = 2) -> Path:
    fs = 1000.0
    t = np.arange(n_samples) / fs
    rng = np.random.default_rng(223)
    signals = []
    for i in range(n_channels):
        signals.append(np.sin(2 * np.pi * (7 + i * 4) * t) + 0.08 * rng.normal(size=t.size))
...
def test_mfdfa_shuffle_registered_and_gui() -> None:
    methods = {m["id"]: m for m in list_advanced_methods()}
    assert "manual_mfdfa_shuffle_surrogate" in methods
    spec = methods["manual_mfdfa_shuffle_surrogate"]
    ...
    assert "mfdfaShuffleTab" in html
    assert "MFDFA Shuffle Surrogate" in html
    assert "manual_mfdfa_shuffle_surrogate" in js
    assert "renderMfdfaShufflePanel" in js
    assert "/mfdfa-shuffle" in server


def test_mfdfa_shuffle_runs_all_available_channels_and_saves_outputs(tmp_path: Path) -> None:
    root = _make_recording(tmp_path)
    result = run_advanced_method(
        "manual_mfdfa_shuffle_surrogate",
        root,
        {"mode": "ultra", "n_shuffles": 2, "max_channels": 2, "random_seed": 7},
    )
    assert result["ok"] is True
    payload = result["result"]["manual_mfdfa_shuffle_surrogate"]
    assert payload["summary"]["mode"] == "ultra"
    assert payload["summary"]["n_channels"] == 2
    assert payload["summary"]["n_shuffles"] == 2
    assert len(payload["rows"]) == 2
    assert len(payload["shuffle_rows"]) == 4
    ...
    for key in ("summary_csv", "shuffle_csv", "summary_txt"):
